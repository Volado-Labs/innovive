...
- **GA4 conversion tracking (2026-08-19):** production runs Code Snippets **#37 "GA4 form_submit tracking - HubSpot embedded forms"** (front-end, priority 21). Key event **`form_submit`** (ONCE_PER_SESSION) fires for the Contact-Us (`4207ae0e…`) and Inno+ (`28d17095…`) forms; the HR form (`b29ca828…`) fires **`job_application`** instead, deliberately, so job applicants cannot inflate the only conversion metric the site has. Custom dimensions `form_name` and `form_id` registered on property 544618753.
  - **Why a snippet and not Enhanced Measurement:** `formInteractionsEnabled` was already true and produced ZERO form events in six weeks. HubSpot's newer embed (`js-na2.hsforms.net/forms/embed/246069906.js` + `div.hs-form-frame`) renders the form in a **cross-origin iframe**, which GA4 Enhanced Measurement cannot observe. #37 listens for HubSpot's `hsFormCallback` / `onFormSubmitted` postMessage instead.
  - **Origin-checked:** only `*.hsforms.net|.hsforms.com|.hubspot.com|.hubapi.com` messages are honoured, so the Google Maps iframe on /contact/ (or any injected script) cannot manufacture a conversion. **Consent-gated for free:** it emits only via `window.gtag`, which snippet #7 defines ONLY after CookieYes analytics consent.
...
- **LinkedIn Insight Tag (2026-09-08):** production runs Code Snippets **#40 "LinkedIn Insight Tag - innovive.com (consent-gated, production only)"** (front-end, priority 20). Partner ID **9263068**, LinkedIn ad account 514047946. Built to mirror snippet #7 exactly: host-gated to `innovive.com` so staging stays dark, and emits nothing until the `cookieyes-consent` cookie carries `advertisement:yes`. A `cookieyes_consent_update` listener means a visitor who accepts *after* landing is still picked up without a reload, which was verified live.
  - **LinkedIn's `<noscript>` beacon is deliberately omitted.** LinkedIn's official snippet ships an `<img src="https://px.ads.linkedin.com/collect/?pid=9263068&fmt=gif">` inside `<noscript>`. That fires regardless of JavaScript and therefore regardless of consent, which would defeat the gate the rest of the snippet exists to enforce. Do not paste LinkedIn's snippet in verbatim if this is ever rebuilt.
  - **`window.lintrk` shim is kept** (LinkedIn's queue stub, defined inside the gate) so conversions can be fired programmatically later, e.g. mirroring #37's HubSpot form hook against conversion rule 28151420. It never exists for a visitor who has not consented.
  - **Snippet #39 "classify LinkedIn as CookieYes Advertisement"** was activated first. It files six cookies (`li_fat_id`, `lms_ads`, `UserMatchHistory`, `AnalyticsSyncHistory`, `li_sugr`, `bcookie`) under CookieYes's `advertisement` category so the preference centre describes them accurately, then pushes the inventory to the CookieYes cloud. Idempotent, self-disables via option `volado_cookieyes_linkedin_advertisement_v1`.
  - **Verified live 2026-09-08** in a real browser: with `advertisement:no` no LinkedIn request fires at all; after Accept All, `snap.licdn.com/li.lms-analytics/insight.min.js` loads and `px.ads.linkedin.com/collect?...&pid=9263068` fires, and `lintrk.q` is replaced by LinkedIn's real implementation. Source of truth: `content/snippets/40-linkedin-insight-tag.php`.
  - **Consent caveat to state whenever these numbers are reported:** LinkedIn only ever sees conversions from visitors who accept advertising cookies, so counted conversions run below true conversions. Cache note: as with any wp_head change, reactivate #10 to flush SiteGround dynamic cache, and expect the uncached page to be correct before the cached one is.
