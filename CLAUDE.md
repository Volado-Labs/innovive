...
- **GA4 conversion tracking (2026-08-19):** production runs Code Snippets **#37 "GA4 form_submit tracking - HubSpot embedded forms"** (front-end, priority 21). Key event **`form_submit`** (ONCE_PER_SESSION) fires for the Contact-Us (`4207ae0e…`) and Inno+ (`28d17095…`) forms; the HR form (`b29ca828…`) fires **`job_application`** instead, deliberately, so job applicants cannot inflate the only conversion metric the site has. Custom dimensions `form_name` and `form_id` registered on property 544618753.
  ...
  - **Pre-existing redirect gaps found while sweeping #9, NOT fixed:** 5 of the 67 legacy Wix entries never reach their mapped target because a WooCommerce product slug shadows the path and WP's canonical redirect wins on `template_redirect`. `/mouse-racks/` goes to `/product/mouse-racks/` (a real product, 200) instead of `/innorack-mouse/`; `/r-wire-hb/`, `/r-feed-u/`, `/r-wire-td/` and `/r-wbp/` all dump the visitor on the **homepage** instead of `/rat-caging/` (their `/product/<slug>/` URLs 404, so these are likely non-published products). Two `/news/...` entries correctly do not redirect: that is the deliberate `is_singular('news')` skip, and those map rows are simply dead.
