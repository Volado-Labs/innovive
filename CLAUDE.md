...
- **GA4 conversion tracking (2026-08-19):** production runs Code Snippets **#37 "GA4 form_submit tracking - HubSpot embedded forms"** (front-end, priority 21). Key event **`form_submit`** (ONCE_PER_SESSION) fires for the Contact-Us (`4207ae0e…`) and Inno+ (`28d17095…`) forms; the HR form (`b29ca828…`) fires **`job_application`** instead, deliberately, so job applicants cannot inflate the only conversion metric the site has. Custom dimensions `form_name` and `form_id` registered on property 544618753.
  ...
  - **Content bug found while fixing this, NOT changed:** `/job-application-form/` (page 3965) embeds the **Contact-Us** GUID `4207ae0e…`, not the HR form, so job applicants submitting there will be counted as `form_submit`, which is exactly the inflation the two-event split exists to prevent. There are two published pages both titled "Job Application" (3268 legacy embed + HR form, 3965 v4 embed + Contact-Us form) and neither is linked from `/career/` or the homepage. Needs an owner decision.
