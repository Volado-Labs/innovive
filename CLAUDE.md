...
- **GA4 conversion tracking (2026-08-19):** production runs Code Snippets **#37 "GA4 form_submit tracking - HubSpot embedded forms"** (front-end, priority 21). Key event **`form_submit`** (ONCE_PER_SESSION) fires for the Contact-Us (`4207ae0e…`) and Inno+ (`28d17095…`) forms; the HR form (`b29ca828…`) fires **`job_application`** instead, deliberately, so job applicants cannot inflate the only conversion metric the site has. Custom dimensions `form_name` and `form_id` registered on property 544618753.
  - **v3 (2026-09-08) — THE ORIGINAL v1 NEVER FIRED ONCE.** GA4 recorded zero `form_submit` and zero `job_application` from 2026-08-19 to 2026-09-08 (~1,050 sessions), and in fact only four event types have ever existed on the property. Two independent faults, both in v1:
    1. **Wrong channel for the forms that carry the volume.** v1 bound `window.addEventListener('message')` for `{type:'hsFormCallback', eventName:'onFormSubmitted'}`, which is the **legacy v2 embed** API. `/contact/`, `/inno-plus/` and `/job-application-form/` run the **v4 embed** (`js-na2.hsforms.net/forms/embed/246069906.js` + `div.hs-form-frame`), which transfers a `MessagePort` into its iframe and does all messaging over a **private MessageChannel**. Nothing is ever posted to the host window, so no window listener can see it. Verified: the v4 bundle contains zero occurrences of `hsFormCallback`/`onFormSubmitted`; a full live render of `/contact/` produced exactly one window message, `recaptcha-setup` from google.com.
    2. **The origin check rejected the one page that did use the legacy channel.** `/job-application/` (page 3268, the HR form) really does run the legacy v2 embed (`embed/v2.js` + `hbspt.forms.create`) and really does post `hsFormCallback`. But the v2 embed relays those from the **parent page**, so `ev.origin` is `https://innovive.com`, not an hsforms domain. v1's allowlist threw them all away. Verified live: seven `hsFormCallback` messages during one render, every one of them origin `https://innovive.com`.
  - **The site runs BOTH embeds, so #37 v3 binds both channels:** the v4 `hs-form-event:on-submission:success` bubbling CustomEvent (`detail.formId`), and the legacy `hsFormCallback`/`onFormSubmitted` window message. A shared `emit()` dedupes the same form GUID within 5s so one submission can never count twice. Snippet #38 (UTM bridge, live 2026-09-01) already used the correct v4 event, which is what pointed at the bug.
  - **Spoofing posture:** neither channel carries a usable third-party origin, so v4 events are authenticated with `HubSpotFormsV4.getFormFromEvent(ev)` (returns `undefined` for a fabricated `instanceId`, a live form object for a genuine one) and legacy events require same-origin **plus** a form GUID on the allowlist. Any script already on the page could call `gtag()` directly anyway, so this is parity, not a relaxation. **Consent-gated for free:** it emits only via `window.gtag`, which snippet #7 defines ONLY after CookieYes analytics consent. Verified on the live deployed code: spoofed instanceId rejected, genuine accepted, duplicate suppressed, unknown GUID and non-submit lifecycle events ignored.
  - **Still open:** whether `form_submit` is actually flagged as a **key event** on 544618753. The GA4 MCP is Data API only, and no event row exists yet to inspect. The line above claiming ONCE_PER_SESSION is unverified and the Aug dashboard notes say the property's only key event is the auto-created `purchase`. Check it in the GA4 UI.
  - **`/job-application-form/` (page 3965) fixed 2026-09-08.** It embedded the **Contact-Us** GUID while its own heading read "Job Application Form / attach your resume", so applicants there were counted as `form_submit`. **The HR form cannot be rendered by the v4 embed:** swapping only the GUID produced an empty form and a console `404` on `forms-na2.hsforms.com/embed/v4/render-definition/246069906/b29ca828-…` plus "Container error loading form". That 404 is why page 3268 uses the legacy embed for this form. The fix was therefore to replace the widget's whole embed with the **legacy v2 markup lifted verbatim from 3268's widget `b2f4c81`**, not just to change the GUID. Verified live: the form renders with its real fields including the **Resume / CV** upload and "Submit Application" button, the genuine legacy channel emits seven lifecycle messages for `b29ca828`, lifecycle events fire no gtag, and a submitted message emits `job_application` with `page_path: /job-application-form/`. Backup: `content/backups/page-3965-2026-09-08-pre-hrformguid.json`.
  - **`/job-application-form/` retired 2026-09-08 (301 to `/job-application/`).** 3965 duplicated 3268 and nothing linked to it. Implemented in snippet **#9**, but NOT as a row in the base64 Wix map: that map matches the whole request path, so it only ever catches the ROOT url, and Rank Math's `page-sitemap.xml` was advertising this page in **8 languages** (root + ar/de/es/fr/ko/pt/zh), all returning 200. Added a separate **language-aware `$volado_retired` block** instead. The 67-entry Wix map is byte-identical to before; a sweep of all 68 paths shows **zero behaviour changes**. All 8 variants now 301 in a single hop, each staying in its own language. Backups: `content/backups/snippet-9-2026-09-08-pre-jobappform-redirect.json` (pre-change) and `snippet-9-2026-09-08-v2-deployed.php`.
    - **Trap, hit live and corrected:** do NOT re-add the language prefix to the redirect target. TranslatePress already filters `home_url()` to the current language, so `home_url('/ar/x/')` on an `/ar/` request yields `/ar/ar/x/`. Strip the prefix for the map lookup, then pass the bare slug.
  - **5 shadowed redirects fixed 2026-09-09 by moving snippet #9 from `template_redirect` to the `wp` hook (priority 0).** Two DIFFERENT causes, not one (an earlier note in this file blamed WooCommerce for all five; that was wrong):
    1. `/r-wire-hb/`, `/r-feed-u/`, `/r-wire-td/`, `/r-wbp/` are **parentless MEDIA ATTACHMENTS** (ids 2579, 2587, 2585, 2586) whose permalink is literally that path. **Rank Math's attachment redirect** sent all four to the homepage (`x-redirect-by: Rank Math`). There is no `wp_rank_math_redirections` table on this install, so this is the attachment setting, NOT a stored redirect rule; do not go looking for one.
    2. `/mouse-racks/` is a **published WooCommerce product** (id 208), so core's `redirect_canonical` sent it to `/product/mouse-racks/`.
  - **Why the `wp` hook:** `template_redirect` priority 0 was tried first and beat core's `redirect_canonical` (fixing mouse-racks) but still LOST to Rank Math, which hooks earlier. `wp` fires inside `WP::main()` before `template_redirect` entirely, and the query is already parsed there so the `is_singular('news')` exception still works. Verified: 61/61 non-news entries reach their mapped target, 0 wrong; the 2 real News CPT posts still serve 200 un-redirected while the 5 dead `/news/` rows still 301 to `/blog-news/`; `/product/mouse-racks/` stays reachable at its own canonical URL. Only those 5 paths changed vs the pre-session baseline.
  - **`x-redirect-by` is the fast diagnostic** for "my redirect is not firing": it names whoever actually issued the 301 (`Rank Math`, `WordPress`, etc). `wp_redirect()` from a snippet reports as `WordPress`.
  - Backup of the deployed snippet: `content/backups/snippet-9-2026-09-09-v3-deployed.php`.
  - **Rule of thumb this established:** the HR form `b29ca828…` is legacy-v2-only. Do NOT move it to the v4 `div.hs-form-frame` embed; it will render blank. The Contact-Us and Inno+ forms are v4.
  - v3 source: `content/backups/snippet-37-2026-09-08-v3-deployed.php`; v1 backup: `content/backups/snippet-37-2026-09-08-pre-v2-customevent.json`.
  - Naming is deliberate: `form_submit`, NOT `generate_lead`. GA4 cannot tell a sales inquiry from a service request; only HubSpot can, once the `inquiry_type` property exists. Backup: `content/backups/snippet-37-2026-08-19-ga4-form-tracking.json`; snippet #7 pre-change backup: `content/backups/snippet-7-2026-08-19-pre-formsubmit.json`.
  - Cache note: after activating #37 the page still served the old HTML until snippet #10 was reactivated to flush SiteGround dynamic cache. Expect that for any wp_head change.
